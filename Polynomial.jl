...
# building_blocks = Vector{Expr}([quote x1 end, quote x2 end, quote x3 end, quote σ end, quote ρ end, quote β end]);
building_blocks = Vector{Expr}([:(x1 .+ 0.0), :(x2 .+ 0.0), :(β + 0.0)]);
...
function compute_predictions(m::Expr)
    y_hat = eval(m)
...
function compute_MSE_instantaneous(m::Expr)
    ...
    y_hat = compute_predictions(m) # the expression is evaluated in the global context
    
    return mean((y_hat .- y).^2)
end;

function simulated_annealing(model::Expr,
                             max_iterations = 100,
...
function compute_MI(model::Expr)
...
function compute_MI_nonlinear(model::Expr)
    # Find the correct parameters
    simulated_annealing(model)
